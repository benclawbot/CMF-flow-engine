...
home_anchor = '''                    onOpenTasks = { tab = ProductTab.Tasks },
                    onEndSession = { id -> scope.launch { endSession(id); notify("Session ended") } },'''
home_replacement = '''                    onOpenTasks = { tab = ProductTab.Tasks },
                    onCompleteTask = { id ->
                        scope.launch {
                            completeTask(id)
...
if home_anchor not in s:
    raise SystemExit('Home callback anchor not found')
s = s.replace(home_anchor, home_replacement, 1)

old_task_start = '''                    onStart = { task ->
                        scope.launch {
                            if (session == null) {
                                startSession(
                                    SessionEntity(
...
if old_task_start not in s:
    raise SystemExit('Task start anchor not found')
s = s.replace(old_task_start, new_task_start, 1)

old_experiment_call = '''                ProductTab.Experiments -> ProductExperimentsScreen(
                    experiments = experiments,
                    assignments = assignments,
                    reports = reports,
                    openTrial = openTrial,
                    onAdd = { exp -> scope.launch { addExperiment(exp); notify("Experiment created") } },'''
new_experiment_call = '''                ProductTab.Experiments -> ProductExperimentsScreen(
                    experiments = experiments,
                    assignments = assignments,
                    reports = reports,
                    contexts = contexts,
...
if old_experiment_call not in s:
    raise SystemExit('Experiment call anchor not found')
s = s.replace(old_experiment_call, new_experiment_call, 1)

sig_anchor = '''    onStartTask: () -> Unit,
    onOpenTasks: () -> Unit,
    onEndSession: (Long) -> Unit,'''
sig_replacement = '''    onStartTask: () -> Unit,
    onOpenTasks: () -> Unit,
    onCompleteTask: (Long) -> Unit,
    onEndSession: (Long) -> Unit,'''
if sig_anchor not in s:
    raise SystemExit('Home signature anchor not found')
s = s.replace(sig_anchor, sig_replacement, 1)

session_actions = '''                    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
...
session_actions_new = '''                    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        OutlinedButton(onClick = { onStruggle(session.id) }) { Text("I'm stuck") }
...
if session_actions not in s:
    raise SystemExit('Session actions anchor not found')
s = s.replace(session_actions, session_actions_new, 1)

best_task_button = '''                    Button(onClick = onStartTask) {
                        Icon(Icons.Filled.PlayArrow, null)
                        Spacer(Modifier.size(6.dp))
                        Text("Start focus")
                    }'''
best_task_button_new = '''                    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
...
if best_task_button not in s:
    raise SystemExit('Best-task action anchor not found')
s = s.replace(best_task_button, best_task_button_new, 1)

insights_start = s.index('@Composable\nprivate fun ProductInsightsScreen')
tasks_start = s.index('@Composable\nprivate fun ProductTasksScreen', insights_start)
insights = '''@Composable
...
'''
s = s[:insights_start] + insights + s[tasks_start:]

s = s.replace('ScreenHeader("Tasks", "A state-aware queue, not a static to-do list.")',
              'ScreenHeader("Tasks", "The same state-aware queue used on Home. Changes update both views.")', 1)

experiments_start = s.index('@Composable\nprivate fun ProductExperimentsScreen')
settings_start = s.index('@Composable\nprivate fun ProductSettingsScreen', experiments_start)
experiments_screen = '''@Composable
private fun ProductExperimentsScreen(
...
'''
s = s[:experiments_start] + experiments_screen + s[settings_start:]

old_metric = '''            Text(label, color = MaterialTheme.colorScheme.onSurfaceVariant, style = MaterialTheme.typography.bodySmall)
            Text(value, style = MaterialTheme.typography.headlineSmall, fontWeight = FontWeight.Bold)'''
new_metric = '''            Text(label, color = Color(0xFF3B3543), style = MaterialTheme.typography.bodySmall, fontWeight = FontWeight.Medium)
...
if old_metric not in s:
    raise SystemExit('Insight metric anchor not found')
